...
import { fetchApplicationList, createApplication, type CreateApplicationPayload } from '@/service/modules/applications'
import { allData } from '@/service/modules/issues'
...
export function Projects() {
  const {
    ...
    queryFn: async () => {
      ...
      const { data: allEventsData } = await allData()
  ...
  const handleCreateApplication = async (data: CreateApplicationPayload) => {
    try {
  ...
  const emptyContent = (
    <div className="flex flex-col h-[calc(100vh-200px)] items-center justify-center space-y-4">
      ...
      <CreateProjectsModal onCreateProject={handleCreateApplication} />
    </div>
  )
